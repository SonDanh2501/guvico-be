...
import { AVG_COLLABORATOR_STAR, AVG_ORDER_STAR, Collaborator, CollaboratorDocument, Customer, CustomerDocument, DeviceToken, DeviceTokenDocument, ERROR, GlobalService, ID_ORDER, LOOKUP_COLLABORATOR, ORDER_STAR, Order, OrderDocument, PunishCollaboratorDTOAdmin, TEMP_COLLABORATOR_DATE_START, TEMP_ORDER_DATE_WORK, TIME_DIFFERRENT, TOTAL_HOUR, TOTAL_LATE_START, TOTAL_ORDER, collaboratorReward, iPageDTO, payloadCreateInfoRewardCollaborator } from 'src/@core';
...
import { addHours, endOfMonth, startOfMonth, subDays } from 'date-fns';
import { InfoRewardCollaboratorService } from '../info-reward-collaborator/info-reward-collaborator.service';

@Injectable()
export class RewardCollaboratorManagerService {
    constructor(
    ...
    async getList(lang, iPage: iPageDTO) {
        try {
            ...
            const arrItem = await this.rewardCollaboratorModel.find(query)
    ...
    async createReward(lang, payload: createRewardCollaboratorDTOAdmin, idAdmin: string) {
        try {
            const newItem = new this.rewardCollaboratorModel({
                title: payload.title,
                ...
                is_limit_total_received: payload.is_limit_total_received,
                limit_total_received: payload.limit_total_received,
                status: payload.status,
                is_type_collaborator: payload.is_type_collaborator,
                type_collaborator: payload.type_collaborator,
                is_city: payload.is_city,
                city: payload.city,
                timezone: payload.timezone,
                condition: payload.condition,
                is_service_apply: payload.is_service_apply,
                service_apply: payload.service_apply,
                type_wallet: payload.type_wallet,
                is_auto_verify: payload.is_auto_verify,
                date_create: new Date(Date.now()).toISOString(),
            });
            await newItem.save();
            this.activityAdminSystemService.createRewardCollaborator(idAdmin, newItem._id);
            return newItem;
        } catch (err) {
            throw new HttpException(err.response || [{ message: err.toString(), field: null }], err.status || HttpStatus.FORBIDDEN);
        }
    ...
    async editReward(lang, payload: editRewardCollaboratorDTOAdmin, id: string, idAdmin: string) {
        try {
            const getItem = await this.rewardCollaboratorModel.findById(id);
            if (!getItem) {
            ...
            getItem.is_limit_date = (payload.is_limit_date === false && getItem.is_limit_date) ? false : true;
            getItem.start_date = payload.start_date || getItem.start_date;
            getItem.end_date = payload.end_date || getItem.end_date;
            getItem.status = payload.status || getItem.status;
            getItem.is_type_collaborator = (payload.is_type_collaborator === false && getItem.is_type_collaborator) ? false : true;
            getItem.type_collaborator = payload.type_collaborator || getItem.type_collaborator;
            getItem.is_city = (payload.is_city === false && getItem.is_city) ? false : true;
            getItem.city = payload.city || getItem.city;
            getItem.timezone = payload.timezone || getItem.timezone;
            getItem.condition = payload.condition || getItem.condition;
            getItem.total_received = payload.total_received || getItem.total_received;
            getItem.is_limit_total_received = (payload.is_limit_total_received !== getItem.is_limit_total_received) ? payload.is_limit_total_received : getItem.is_limit_total_received;
            getItem.limit_total_received = payload.limit_total_received || getItem.limit_total_received;
            getItem.is_service_apply = (payload.is_service_apply !== getItem.is_service_apply) ? payload.is_service_apply : getItem.is_service_apply;
            getItem.service_apply = payload.service_apply || getItem.service_apply;
            getItem.type_wallet = payload.type_wallet || getItem.type_wallet;
            getItem.is_auto_verify = payload.is_auto_verify || getItem.is_auto_verify
            await getItem.save();
            this.activityAdminSystemService.editRewardCollaborator(idAdmin, id);
            return getItem;
        } catch (err) {
            throw new HttpException(err.response || [{ message: err.toString(), field: null }], err.status || HttpStatus.FORBIDDEN);
        }
    ...
    async actiReward(lang, payload: actiRewardCollaboratorAdmin, id: string, idAdmin: string) {
        try {
            const getItem = await this.rewardCollaboratorModel.findById(id);
            ...
            await getItem.save();
            this.activityAdminSystemService.activeRewardCollaborator(idAdmin, id);
            return true;
        } catch (err) {
            throw new HttpException(err.response || [{ message: err.toString(), field: null }], err.status || HttpStatus.FORBIDDEN);
        }
    ...
    async deleteReward(lang, payload: deleteRewardCollaboratorAdmin, id: string, idAdmin: string) {
        try {
            const getItem = await this.rewardCollaboratorModel.findById(id);
            if (!getItem) throw new HttpException([await this.customExceptionService.i18nError(ERROR.ITEM_NOT_FOUND, lang, null)], HttpStatus.NOT_FOUND);
            getItem.is_delete = true;
            await getItem.save();
            this.activityAdminSystemService.deleteRewardCollaborator(idAdmin, id);
            return true;
        } catch (err) {
            throw new HttpException(err.response || [{ message: err.toString(), field: null }], err.status || HttpStatus.FORBIDDEN);
        }
    ...
    async getDetailReward(lang, idReward) {
        try {
            const getDetailQuestion = await this.rewardCollaboratorModel.findById(idReward);
            if (!getDetailQuestion) throw new HttpException([await this.customExceptionService.i18nError(ERROR.ITEM_NOT_FOUND, lang, null)], HttpStatus.NOT_FOUND);
