...
import { API_KEY } from './goong.config';
@Injectable()
    ...
    constructor(
        ...
        public async autoComplete(input, idUser) {
            try {
                ...
                const url = `https://rsapi.goong.io/Place/AutoComplete?api_key=${API_KEY}&input=${temp}`
        ...
        public async placeDetail(id, idUser) {
            try {
                ...
                const url = `https://rsapi.goong.io/Place/Detail?place_id=${id}&api_key=${API_KEY}`
        ...
        public async geocode(latlng, idUser) {
            try {
                ...
                const url = `https://rsapi.goong.io/Geocode?latlng=${split[0]},${split[1]}&api_key=${API_KEY}`
