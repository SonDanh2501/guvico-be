...
import { DEFAULT_LANG, editAddressDTOAdmin, iPageDTO, LANGUAGE, ValidateLangPipe, GlobalService, GetUserByToken, createAddressDTOCollaborator, editAddressDTOCollaborator } from 'src/@core';
...
export class AddressController {
    ...
    async createItem(
        @Query('lang', ValidateLangPipe) lang: LANGUAGE = DEFAULT_LANG,
        @Body() req: createAddressDTOCollaborator,
        @GetUserByToken() user
    ) {
        try {
            const payload = req;
            const result = await this.addressService.createItem(lang, payload, user);
            return result;
        } catch (err) {
    ...
    async editItem(
        ...
        @GetUserByToken() user,
        @Param('id') id: string,
        @Body() req: editAddressDTOCollaborator,
    ) {
    ...
    async deleteAddress(
        ...
        const result = await this.addressService.deleteAddress(lang, id, user);
        return result;
    }
    ...
    async setDefaultAddress(
        ...
        const result = await this.addressService.setDefaultAddress(lang, id, user);
        return result;
    }
}
